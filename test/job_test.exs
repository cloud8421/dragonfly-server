defmodule JobTest do
  ...
  test "creates a command from job payload" do
    ...
    expected = %{fetch: "#{System.get_env("HTTP_HOST")}/attachments/20140123T175745-2823/Untitled.png",
      shell: "#{TestConfig.convert_command} - -thumbnail 273x273^^ -gravity center -crop 273x273+0+0 +repage -draw 'polygon 0,0 273,273 273,0 fill none matte 135,135 floodfill' -strip png:-",
      format: "png"}
  ...
  test "supports local files" do
    ...
    expected = %{file: "/app/app/assets/images/default_article_image.png", format: "jpg", shell: "#{TestConfig.convert_command} - -thumbnail 100x100# -strip jpg:-"}
    assert(expected == Job.to_command(payload))
  end
